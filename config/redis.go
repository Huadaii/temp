...
func (c *Cacher) Keys(pattern string) ([]string, error) {
	return Strings(c.Do("KEYS", pattern))
}
...
// SetNX 存并设置有效时长。时长的单位为秒。
// 如果存在返回false,否则直接返回
func (c *Cacher) SetNX(key string, val interface{}, expire int64) error {
	value, err := c.encode(val)
	if err != nil {
		return err
	}
	if expire > 0 {
		_, err := String(c.Do("SET", c.getKey(key), value, "EX", expire, "NX"))
		return err
	}
	_, err = String(c.Do("SET", c.getKey(key), value, "NX"))
	return err
}
